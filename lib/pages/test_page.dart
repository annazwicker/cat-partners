        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            StreamBuilder(
              ...
              {
                if(!snapshot.hasData) {
                  return const Center(child: Text('the bogo bingus'));
                }
                List<QueryDocumentSnapshot<UserDoc>> users = snapshot.data?.docs ?? [];
                
                return ElevatedButton(
                  onPressed: () {
                    for(QueryDocumentSnapshot<UserDoc> user in users) {
                      _dbHelper.db.runTransaction(
                        (transaction) async {
                          UserDoc newUser = user.data().copyWith(
                            name: user.data().getName(),
                            affiliation: '',
                            rescueGroup: '',
                            phoneNumber: '',
                            email: '');
                          transaction.set(user.reference, newUser);
                        });
                    }
                    // for (int i = 1; i <= 30; i++){
